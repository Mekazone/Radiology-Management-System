...
//get report info, and add link to edit info
	...
	$mri_reporter_id = $row5['mri_reporter_id'];
	...
	$row5_mri_report = @nl2br(ucfirst($row5['mri_report']));
	$row5_mri_radiologist_diagnosis = @nl2br(ucfirst($row5['mri_radiologist_diagnosis']));
	$row5_mri_differential = @nl2br(ucfirst($row5['mri_differential']));
	$row5_mri_further_investigations = @nl2br(ucwords($row5['mri_further_investigations']));
	...
	if($query5_numrows != 0)
	{
		...
		$download_link = "reports/$patient_name/$folder_date/mri/$row4_surname-$row4_first_name-$row4_middle_name-mri.pdf";
	...
	if($row_status == 'admin'){
	echo "<li><a onclick =\"return confirm('Are you sure you want to delete?')\" href='delete_report.php?id=$id&date=$date&modality=mri'>Delete</a></li>";
	}
	//create a link to email report
	echo "<li><a href='email_report.php?id=$id&date=$date&modality=mri'>Email Report<a></li>";
...
$sql3 = "SELECT * FROM images WHERE patient_id='$id' AND modality='mri' AND report_date='$date'";
$query3 = @mysql_query($sql3);
$query3_numrows = @mysql_num_rows($query3);

if($query3_numrows == 0)
{
	echo "<li><a href='images.php?id=$id&date=$date&modality=mri'>No Images/Files, Click to Upload</a></li>";
}
else
{
	echo "<li><a href='images.php?id=$id&date=$date&modality=mri'>View Images/Files</a></li>";
}
...
$row4 = @mysql_fetch_array($query4);
	...
	echo "<tr><td><b>Clinical Diagnosis</b></td><td>$row5_mri_clinical_diagnosis</td></tr>";
	echo "<tr><td><b>Type of Investigation</b></td><td>$row5_mri_invest_type</td></tr>";
	echo "<tr><td><b>Clinician Name</b></td><td>$row5_mri_clinician_name</td></tr>";
	echo "<tr><td><b>Clinician Address</b></td><td>$row5_mri_clinician_address</td></tr>";
	echo "<tr><td><b>Clinician Tel</b></td><td>$row5_mri_clinician_tel</td></tr>";
	echo "<tr><td><b>Report</b></td><td>$row5_mri_report</td></tr>";
	echo "<tr><td><b>Radiological Diagnosis</b></td><td>$row5_mri_radiologist_diagnosis</td></tr>";
	
	//if differentials were entered, print it out
	if(!empty($row5_mri_differential))
	{
	echo "<tr><td><b>Differentials</b></td><td>$row5_mri_differential</td></tr>";
	}
	
	//if further investigations were entered, print it out
	if(!empty($row5_mri_further_investigations))
	{
	echo "<tr><td><b>Further Investigations Recommended</b></td><td>$row5_mri_further_investigations</td></tr>";
	}
?>
