...
//check the database if report has been entered, and add link to edit info
	...
	$lab_reporter_id = $row5['lab_reporter_id'];
	...
	$row5_lab_report = @nl2br(ucfirst($row5['lab_report']));
	$row5_lab_radiologist_diagnosis = @nl2br(ucwords($row5['lab_radiologist_diagnosis']));
	$row5_lab_differential = @nl2br(ucfirst($row5['lab_differential']));
	$row5_lab_further_investigations = @nl2br(ucwords($row5['lab_further_investigations']));
	...
	if($query5_numrows != 0)
	{
		...
		$download_link = "reports/$patient_name/$folder_date/lab/$row4_surname-$row4_first_name-$row4_middle_name-lab.pdf";
	...
	if($row_status == 'admin'){
	echo "<li><a onclick =\"return confirm('Are you sure you want to delete?')\" href='delete_report.php?id=$id&date=$date&modality=lab'>Delete</a></li>";
	}
	//create a link to email report
	echo "<li><a href='email_report.php?id=$id&date=$date&modality=lab'>Email Report<a></li>";
...
$sql3 = "SELECT * FROM images WHERE patient_id='$id' AND modality='lab' AND report_date='$date'";
$query3 = @mysql_query($sql3);
$query3_numrows = @mysql_num_rows($query3);

if($query3_numrows == 0)
{
	echo "<li><a href='images.php?id=$id&date=$date&modality=lab'>No Images/Files, Click to Upload</a></li>";
}
else
{
	echo "<li><a href='images.php?id=$id&date=$date&modality=lab'>View Images/Files</a></li>";
}
...
$row4 = @mysql_fetch_array($query4);
	...
	echo "<tr><td><b>Clinical Diagnosis</b></td><td>$row5_lab_clinical_diagnosis</td></tr>";
	echo "<tr><td><b>Type of Investigation</b></td><td>$row5_lab_invest_type</td></tr>";
	echo "<tr><td><b>Clinician Name</b></td><td>$row5_lab_clinician_name</td></tr>";
	echo "<tr><td><b>Clinician Address</b></td><td>$row5_lab_clinician_address</td></tr>";
	echo "<tr><td><b>Clinician Tel</b></td><td>$row5_lab_clinician_tel</td></tr>";
	echo "<tr><td><b>Type of Investigation</b></td><td>$row5_lab_invest_type</td></tr>";
	echo "<tr><td><b>Report</b></td><td>$row5_lab_report</td></tr>";
	echo "<tr><td><b>Lab Diagnosis</b></td><td>$row5_lab_radiologist_diagnosis</td></tr>";
	
	//if differentials were entered, print it out
	if(!empty($row5_lab_differential))
	{
	echo "<tr><td><b>Differentials</b></td><td>$row5_lab_differential</td></tr>";
	}
	
	//if further investigations were entered, print it out
	if(!empty($row5_lab_further_investigations))
	{
	echo "<tr><td><b>Further Investigations Recommended</b></td><td>$row5_lab_further_investigations</td></tr>";
	}
?>
