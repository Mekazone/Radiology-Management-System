...
//process form
$submit_month = $_GET['submit_month'];
$month = $_GET['month'];
$year = $_GET['year'];
...
if($submit_month)
{
	...
	elseif(!$sort_error)
	{
		//create query according to month (january to dec)
		if($month == "4" || $month == "6" || $month == "9" || $month == "11")
		{
			//30 days
			$start_month = adodb_mktime(0,0,0,$month,1,$year);
			$end_month = adodb_mktime(0,0,0,$month,30,$year);
		}
		elseif($month == "2" )
		{
			...
			if (is_leap_year($year) == 'yes')
			{
				//29 days
				$start_month = adodb_mktime(0,0,0,$month,1,$year);
				$end_month = adodb_mktime(0,0,0,$month,29,$year);
			}
			else
			{
				//28 days
				$start_month = adodb_mktime(0,0,0,$month,1,$year);
				$end_month = adodb_mktime(0,0,0,$month,28,$year);
			}
		}
		else
		{
			//31 days
			$start_month = adodb_mktime(0,0,0,$month,1,$year);
			$end_month = adodb_mktime(0,0,0,$month,31,$year);
		}
		
		//create query		
		//display sort result
		//get total number of rows to be used for pagination
$sql1 = "SELECT investigation_payment.*,patients.id,patients.surname,patients.first_name,patients.middle_name FROM investigation_payment,patients WHERE investigation_payment.patient_id = patients.id AND investigation_payment.date BETWEEN '$start_month' AND '$end_month' ORDER BY investigation_payment.date DESC";
...
$sql3 = "SELECT investigation_payment.*,patients.id,patients.surname,patients.first_name,patients.middle_name FROM investigation_payment,patients WHERE investigation_payment.patient_id = patients.id AND investigation_payment.date BETWEEN '$start_month' AND '$end_month' ORDER BY investigation_payment.date DESC LIMIT $results,$row_per_page";
...
<div id="main_centre">
<h3>Sort Billing by Month</h3>

<!-- Create means to sort results-->
<table id="monitor_billing">
<form method="GET" action="">
<tr>
<td style="background:#ddd;padding: 5px 10px;border:1px solid #000">Enter Month</td>
<td style="padding: 5px 10px;"><select name='month'><option value=''>Month</option>";
	<?php
	...
	?>
	echo "</select></td>
<td><button name="submit_month" onclick="process_notice12()" type="submit" value="Sort" id="button_click">Sort</button></td>
</tr>
</form>
</table>
<div id="wait12"></div>
...
$sql5 = "SELECT investigation_payment.*,patient_billing.billing_id,patient_billing.investigation_name,patients.id,patients.surname,patients.first_name,patients.middle_name FROM investigation_payment,patient_billing,patients WHERE patient_billing.billing_id = investigation_payment.billing_id AND investigation_payment.patient_id = patients.id AND investigation_payment.date BETWEEN '$start_month' AND '$end_month' AND investigation_payment.type = 'balance' ORDER BY investigation_payment.date ASC";
...
if($query5_numrows > 0)
{
	...
	//if net balance is less than 0, patient paid for investigation owed in previous month
	if($net_balance < 0)
	{
		$net_balance = 0;
	}
...
if($query5_numrows > 0)
{
	...
	echo "<div id='total_cost'>";
	echo "<table>";
	$sql6 = "SELECT DISTINCT patient_id FROM investigation_payment WHERE date BETWEEN '$start_month' AND '$end_month'";
	$query6 = mysql_query($sql6);
	while($row6 = mysql_fetch_array($query6))
	{
		$patient_id = $row6['patient_id'];
		
		$sql7 = "SELECT investigation_payment.*,patients.id,patients.surname,patients.first_name,patients.middle_name FROM investigation_payment,patients WHERE investigation_payment.patient_id = patients.id AND investigation_payment.date BETWEEN '$start_month' AND '$end_month' AND patients.id = '$patient_id'";
...
if($page && ($page > 1))
{
	...
	echo "<a href='billing_month.php?page=$i&month=$month&year=$year&submit_month=$submit_month' style='margin-right:20px;'>Previous</a> ";
}
...
if($page<$result_rows && ($result_rows > 1))
{
	...
	echo "<a href='billing_month.php?page=$i&month=$month&year=$year&submit_month=$submit_month' style='margin-left:20px;'>Next</a> ";
}
echo "</div>";
echo "</div>";
}
