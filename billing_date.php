...
$sql1 = "SELECT investigation_payment.*,patients.id,patients.surname,patients.first_name,patients.middle_name FROM investigation_payment,patients WHERE investigation_payment.patient_id = patients.id AND investigation_payment.date = '$date' ORDER BY patients.surname ASC";
...
$sql3 = "SELECT investigation_payment.*,patients.id,patients.surname,patients.first_name,patients.middle_name FROM investigation_payment,patients WHERE investigation_payment.patient_id = patients.id AND investigation_payment.date = '$date' ORDER BY patients.surname ASC LIMIT $results,$row_per_page";
...
<div id="main_centre">
<h3>Sort Billing by Date</h3>

<!-- Create means to sort results-->
<table id="monitor_billing">
<form method="GET" action="">
<tr>
<td style="background:#ddd;padding: 5px 10px;border:1px solid #000">Enter Date</td>
<td style="padding: 5px 10px;"><select name='day'><option value=''>Day</option>";
	<?php
	...
	?>
	echo "</select></td>
<td><button name="submit_date" onclick="process_notice12()" type="submit" value="Sort" id="button_click">Sort</button></td>
</tr>
</form>
</table>
<div id="wait12"></div>
...
$sql4 = "SELECT investigation_payment.*,patients.id,patients.surname,patients.first_name,patients.middle_name FROM investigation_payment,patients WHERE investigation_payment.patient_id = patients.id AND investigation_payment.date = '$date' AND investigation_payment.type = 'fresh' ORDER BY patients.surname ASC";
...
$sql5 = "SELECT investigation_payment.*,patients.id,patients.surname,patients.first_name,patients.middle_name FROM investigation_payment,patients WHERE investigation_payment.patient_id = patients.id AND investigation_payment.date = '$date' AND investigation_payment.type = 'balance' ORDER BY patients.surname ASC";
...
$sql5 = "SELECT investigation_payment.*,patient_billing.billing_id,patient_billing.investigation_name,patients.id,patients.surname,patients.first_name,patients.middle_name FROM investigation_payment,patient_billing,patients WHERE patient_billing.billing_id = investigation_payment.billing_id AND investigation_payment.patient_id = patients.id AND investigation_payment.date = '$date' AND investigation_payment.type = 'balance' ORDER BY investigation_payment.date ASC";
$query5 = mysql_query($sql5);
$query5_numrows = mysql_num_rows($query5);
if($query5_numrows > 0)
{
	...
	echo "<div id='total_cost'>";
	echo "<table>";
	$sql6 = "SELECT DISTINCT patient_id FROM investigation_payment WHERE date = '$date'";
	$query6 = mysql_query($sql6);
	while($row6 = mysql_fetch_array($query6))
	{
		$patient_id = $row6['patient_id'];
		
		$sql7 = "SELECT investigation_payment.*,patients.id,patients.surname,patients.first_name,patients.middle_name FROM investigation_payment,patients WHERE investigation_payment.patient_id = patients.id AND investigation_payment.date = '$date' AND patients.id = '$patient_id'";
...
if($page && ($page > 1))
{
	...
	echo "<a href='billing_date.php?page=$i&day=$day&month=$month&year=$year&submit_date=$submit_date' style='margin-right:20px;'>Previous</a> ";
}
...
if($page<$result_rows && ($result_rows > 1))
{
	...
	echo "<a href='billing_date.php?page=$i&day=$day&month=$month&year=$year&submit_date=$submit_date' style='margin-left:20px;'>Next</a> ";
}
echo "</div>";
echo "</div>";
}
