...
//check the database if report has been entered, and add link to edit info
	...
	$ct_reporter_id = $row5['ct_reporter_id'];
	...
	$row5_ct_invest_type = ucwords($row5['ct_invest_type']);
	$row5_ct_report = @nl2br(ucfirst($row5['ct_report']));
	$row5_ct_radiologist_diagnosis = @nl2br(ucfirst($row5['ct_radiologist_diagnosis']));
	$row5_ct_differential = @nl2br(ucfirst($row5['ct_differential']));
	$row5_ct_further_investigations = @nl2br(ucwords($row5['ct_further_investigations']));
	...
	if($query5_numrows != 0)
	{
		...
		$download_link = "reports/$patient_name/$folder_date/ct/$row4_surname-$row4_first_name-$row4_middle_name-ct.pdf";
	...
	if($row_status == 'admin'){
	echo "<li><a onclick =\"return confirm('Are you sure you want to delete?')\" href='delete_report.php?id=$id&date=$date&modality=ct'>Delete</a></li>";
	}
		//create a link to email report
	echo "<li><a href='email_report.php?id=$id&date=$date&modality=ct'>Email Report<a></li>";
...
$sql3 = "SELECT * FROM images WHERE patient_id='$id' AND modality='ct' AND report_date='$date'";
$query3 = @mysql_query($sql3);
$query3_numrows = @mysql_num_rows($query3);

if($query3_numrows == 0)
{
	echo "<li><a href='images.php?id=$id&date=$date&modality=ct'>No Images/Files, Click to Upload</a></li>";
}
else
{
	echo "<li><a href='images.php?id=$id&date=$date&modality=ct'>View Images/Files</a></li>";
}
...
$row4 = @mysql_fetch_array($query4);
	...
	echo "<tr><td><b>Clinical Diagnosis</b></td><td>$row5_ct_clinical_diagnosis</td></tr>";
	echo "<tr><td><b>Type of Investigation</b></td><td>$row5_ct_invest_type</td></tr>";
	echo "<tr><td><b>Clinician Name</b></td><td>$row5_ct_clinician_name</td></tr>";
	echo "<tr><td><b>Clinician Address</b></td><td>$row5_ct_clinician_address</td></tr>";
	echo "<tr><td><b>Clinician Tel</b></td><td>$row5_ct_clinician_tel</td></tr>";
	echo "<tr><td><b>Report</b></td><td>$row5_ct_report</td></tr>";
	echo "<tr><td><b>Radiological Diagnosis</b></td><td>$row5_ct_radiologist_diagnosis</td></tr>";
	
	//if differentials were entered, print it out
	if(!empty($row5_ct_differential))
	{
	echo "<tr><td><b>Differentials</b></td><td>$row5_ct_differential</td></tr>";
	}
	
	//if further investigations were entered, print it out
	if(!empty($row5_ct_further_investigations))
	{
	echo "<tr><td><b>Further Investigations Recommended</b></td><td>$row5_ct_further_investigations</td></tr>";
	}
?>
